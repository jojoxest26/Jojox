...
describe("MCP server: handleRequest", () => {
  ...
  it("tools/call su analyze_code trova un problema in codice vulnerabile", () => {
    const res = handleRequest({
      ...
      params: {
        name: "analyze_code",
        arguments: { files: [{ path: "supabase/migrations/0001.sql", content: "ALTER TABLE public.orders ENABLE ROW LEVEL SECURITY;" }] },
      },
    }) as any;
  ...
  it("tools/call su fix_code restituisce solo i file effettivamente cambiati", () => {
    const res = handleRequest({
      ...
      params: {
        ...
        arguments: {
          files: [
            { path: "supabase/migrations/0001.sql", content: "ALTER TABLE public.orders ENABLE ROW LEVEL SECURITY;" },
